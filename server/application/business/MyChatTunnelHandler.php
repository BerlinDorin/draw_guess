 ...
 * 实现 WebSocket 信道处理器
 */
class MyChatTunnelHandler implements ITunnelHandler {
  ...
  public function setRoom ($type, $id){
    $roomId = $id;
    switch($type){
      case 'create':
        // $data = self::loadData();
        // $data['userMap'][$id] = $this->userinfo;
        // $roomId = $data['userMap'][$id];
        // $data['$roomId'] = array($id);
        // self::saveData($data);
      break;
      case 'join':
        if(array_key_exists($roomId, $data)){  // 房间存在
          array_push($data, $tunnelId);  // 加入这个人
          $gamerInfo = array();  // 获得这个房间所有人的信息
          for($i = 0; $i < count($data[$roomId]); $i+=1){
            array_push($gamerInfo, $data['userMap'][$data[$roomId][i]]);
          }
          TunnelService::emit($tunnelId, 'all people', array('content' => $gamerInfo));  // 把所有人的信息告诉新加入的这个人
          self::broadcast('new person', array(  // 把新加入的这个人的信息告诉其他所有人
            'total' => count($data[$roomId]),
            'enter' => $data['userMap'][$tunnelId],
          ), true, $tunnelId);
          self::saveData($data);
        }else{
          debug('房间不存在', $tunnelId);
        }
      break;
    }
  }
  ...
  public function onConnect($tunnelId) {
    ...
    if (array_key_exists($tunnelId, $data['userMap'])) {
      ...
      self::saveData($data);
      self::broadcast('new person', array(
  ...
  public function onMessage($tunnelId, $type, $content) {
    $data = self::loadData();
    TunnelService::emit($tunnelId, 'speak', $roomId);
  ...
  public function onClose($tunnelId) {
      ...
      if (count($data[$roomId]) > 0) {
          self::broadcast('someone leave', array(
              'total' => count($data[$roomId]),
              'leave' => $leaveUser,
          ));
          unset($data[$roomId]);
  ...
  private static function mybroadcast($type, $content, $mask=false, $senderTunnelId=0) {
      $data = self::loadData();
      $tunnelIds = array();
      if(!$mask){
        ...
        foreach ($data[$roomId] as $tunnelId){
  ...
  private static function getDataFilePath() {
      return (dirname(__FILE__) . DIRECTORY_SEPARATOR . 'my_chat_data.json');
  }
}
